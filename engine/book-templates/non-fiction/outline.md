# Outline — Book Plan

## Overview

**Title**: [Working title]
**Subtitle**: [Clarifying subtitle]
**Main Idea**: [One sentence — what reader gets]

## Book Structure

### Introduction
**Purpose**: Grab attention, explain why to read

- [ ] Hook (story, statistic, provocative question)
- [ ] Problem the book solves
- [ ] Promise to reader
- [ ] Brief overview of structure
- [ ] About author (why I can write about this)

**Volume**: ~2,000-3,000 words

---

### Part I: [Part Title]
**Focus**: [What this part covers]

#### Chapter 1: [Title]
**Main Thesis**: [One sentence]

**Structure**:
1. Opening story/example
2. Key concept #1
3. Key concept #2
4. Evidence and data
5. Practical application
6. Conclusions and transition

**Sources for chapter**:
- [ ] [Source 1]
- [ ] [Source 2]

**Examples/Cases**:
- [ ] [Example 1]
- [ ] [Example 2]

---

#### Chapter 2: [Title]
**Main Thesis**: [One sentence]

**Structure**:
1. ...
2. ...

---

### Part II: [Part Title]
**Focus**: [What this part covers]

#### Chapter 3: [Title]
...

---

### Conclusion
**Purpose**: Synthesis, call to action, inspiration

- [ ] Brief summary of key ideas
- [ ] Unifying concept
- [ ] Concrete next steps for reader
- [ ] Inspiring finale

**Volume**: ~2,000-3,000 words

---

## Additional Materials

### Appendices
- [ ] Appendix A: [Title] — [Description]
- [ ] Appendix B: [Title] — [Description]

### Glossary
- [ ] Needed: yes/no
- [ ] Approximate number of terms: ___

### Bibliography
- [ ] Citation format: [APA/MLA/Chicago/other]
- [ ] Approximate number of sources: ___

---

## Writing Timeline

| Element | Deadline | Status |
|---------|---------|--------|
| Research | | ⬜ |
| Introduction | | ⬜ |
| Chapter 1 | | ⬜ |
| Chapter 2 | | ⬜ |
| ... | | ⬜ |
| Conclusion | | ⬜ |
| Editing | | ⬜ |

---

## Plan Notes

[Space for notes, alternative ideas, questions]
